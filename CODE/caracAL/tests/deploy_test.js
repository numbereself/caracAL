...
console.log(
  `I am ${character.name} from ${parent.server_region + parent.server_identifier}`,
);
...
setTimeout(() => {
  ...
  const [onliners, offliners] = partition(
    all_chars,
    (x) => x.server || parent.caracAL.siblings.includes(x.name),
  );
